...
function BatchRegistrationPage() {
  ...
  useEffect(() => {
    ...
    const media3 = sum3 / stage3.length;
    ...
    const variant3 = stage3.reduce((total, valor) => total + Math.pow(media3 - Number(valor.nivel3), 2)/stage3.length, 0);

    // standard deviation
    const dp1 = Math.sqrt(variant1).toFixed(2);
    const dp2 = Math.sqrt(variant2).toFixed(2);
    const dp3 = Math.sqrt(variant3).toFixed(2);

    setStage2({
      nivel1: { ...stage2.nivel1, media: media1.toFixed(2), DP: dp1 },
      nivel2: { ...stage2.nivel2, media: media2.toFixed(2), DP: dp2 },
      nivel3: { ...stage2.nivel3, media: media3.toFixed(2), DP: dp3 }
    });
  }, []);
